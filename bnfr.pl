...
print "import com.muchencute.commons.database.OutParam;\n";
print "import com.muchencute.commons.database.ProcedureInvoker;\n";
...
print "import spark.Response;\n";
print "import java.sql.Types;\n";
print "import spark.Route;\n\n";
		...
		if ($_ =~ /^\s*class\s*=\s*(\w+).*/) {
				...
				if ($_ =~ /^\s*(\w+):\s*(\w+):(.*)/) {
						...
						if ($protocol eq "array") {
								...
								if ($returnType ne "arr") {
								    if (isSPOut($paramName)) {
								    	$executed .= "\t\t$protoObject.getResult().put(\"$paramName\", $paramName);\n";
								    } elsif ($paramType eq "num") {
                                        $executed .= "\t\t$protoObject.getResult().put(\"$paramName\", resultSet.getInt(\"".camel2pieces($paramName)."\"));\n";
								    } elsif ($paramType eq "str") {
                                        $executed .= "\t\t$protoObject.getResult().put(\"$paramName\", resultSet.getString(\"".camel2pieces($paramName)."\"));\n";
								    }
								} elsif ($paramType eq "num") {
										$executed .= "\t\t\titem.put(\"$paramName\", resultSet.getInt(\"".camel2pieces($paramName)."\"));\n";
								} elsif ($paramType eq "str") {
				...
				if ($protocol eq "array") {
					$executed .= "$protoObject.getResult().put(item);\n";
					$executed .= "}\n";
				}
